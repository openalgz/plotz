...
namespace plotz
{
   ...
   heatmap_stamp::heatmap_stamp(uint32_t width, uint32_t height, const std::vector<float>& data) noexcept
      : w(width), h(height), buffer(data)
   {
      assert(data.size() == w * h);
   }

   heatmap_stamp::heatmap_stamp(uint32_t radius)
   {
   ...
   heatmap_stamp::heatmap_stamp(unsigned radius, std::function<float(float)> distshape)
   {
      uint32_t d = 2 * radius + 1;
      w = d;
      h = d;
   ...
   heatmap::heatmap(uint32_t width_in, uint32_t height_in) noexcept : width(width_in), height(height_in) {}

   void heatmap::add_point(uint32_t x, uint32_t y) { add_point_with_stamp(x, y, default_heatmap_stamp); }

   void heatmap::add_point_with_stamp(uint32_t x, uint32_t y, const heatmap_stamp& stamp)
   {
   ...
   void heatmap::add_weighted_point(uint32_t x, uint32_t y, float weight)
   {
      add_weighted_point_with_stamp(x, y, weight, default_heatmap_stamp);
   }

   void heatmap::add_weighted_point_with_stamp(uint32_t x, uint32_t y, float weight, const heatmap_stamp& stamp)
   {
   ...
   std::vector<uint8_t> heatmap::render() const { return render(default_color_scheme_data); }

   std::vector<uint8_t> heatmap::render(const auto& colors) const
   {
      float saturation = max_heat > 0.0f ? max_heat : 1.0f;
      return render_saturated(colors, saturation);
   }

   std::vector<uint8_t> heatmap::render_saturated(const auto& colors, float saturation) const
   {
