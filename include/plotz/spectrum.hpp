...
namespace plotz
{
   struct spectrum final
   {
      ...
      spectrum(uint32_t width_in, uint32_t height_in) noexcept
         : width(width_in),
           height(height_in),
           buffer(width_in, 0.0f),
           peak_values(width_in, 0.0f),
           prev_buffer(width_in, 0.0f)
      {}
      ...
      uint32_t width{}, height{}; // Width is number of frequency bins, height is visualization height
      ...
      std::vector<float> buffer; // Buffer to store magnitude values (1D array)
      ...
      {
         const size_t size = (std::min)(width, static_cast<uint32_t>(magnitudes.size()));
      ...
      {
         if (bin >= width) return; // Ignore if outside buffer
      ...
      std::vector<uint8_t> render_saturated(const std::vector<uint8_t>& colors, float saturation) const
      {
         ...
         for (uint32_t x = 0; x < width; ++x) {
            ...
            // Draw the vertical bar based on selected style
            switch (style) {
